...
public class PassportController extends BaseController {
    ...
    private void synchShopcartData(String userId, HttpServletRequest request,
        ...
        if (StringUtils.isBlank(shopcartJsonRedis)) {
            ...
            if (StringUtils.isNotBlank(shopcartJsonCookie)) {
                ...
                for (ShopcartBO shopcartBoRedis : shopcartListRedis) {
                    String redisSpecId = shopcartBoRedis.getSpecId();
                    for (ShopcartBO shopcartBOCookie : shopcartListCookie) {
                        String cookieSpecId = shopcartBOCookie.getSpecId();
                        if (redisSpecId.equals(cookieSpecId)) {
                            // 覆盖购买数量，不累加，参考京东
                            shopcartBoRedis.setBuyCounts(shopcartBOCookie.getBuyCounts());
                            // 把cookieShopcart放入待删除列表，用于最后的删除与合并
                            pendingDeleteList.add(shopcartBOCookie);
                        }
                    }
                }
    ...
    public IMOOCJSONResult login(@RequestBody UserBO userBO,
        ...
        CookieUtils.setCookie(request, response, "user", JsonUtils.objectToJson(usersVO), true);//true 是否加密
